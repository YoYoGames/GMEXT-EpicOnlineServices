...
std::vector<RValue> _SW_GetArrayOfRValues(RValue* arg, int arg_idx, const char* func)
{
	...
	if (KIND_RValue(pV) == VALUE_ARRAY)
	{
		...
		for (int i = 0; GET_RValue(&elem, pV, NULL, i); ++i)
		{
			...
			{
				YYError("%s argument %d [array element %d] incorrect type (%s) expecting a Struct", func, (arg_idx + 1), i, KIND_NAME_RValue(pV));
			}
