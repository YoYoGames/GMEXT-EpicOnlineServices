...
switch(async_load[? "type"])
{
	...
	case "eos_sessions_add_notify_session_invite_accepted":
		...
		var _details_id = eos_sessions_copy_session_handle_by_invite_id(async_load[? "invite_id"])
		eos_sessions_join_session(SessionName, global.product_user_id)
...
case "eos_sessions_join_session":
        ...
        if(async_load[? "status"] == EpicResult.Success)
        {
            ...
            var _struct = eos_sessions_active_session_copy_info(_handle)
            eos_sessions_active_session_release(_handle)
            var buff = buffer_create(256,buffer_fixed,1)
            buffer_write(buff,buffer_u8,1)
            eos_p2p_send_packet(global.product_user_id, _struct.details.owner_user_id, obj_eos_sessions_p2p.socketName, 0, buff, buffer_tell(buff), true, EpicPacketReliability.ReliableOrdered, false)
            buffer_delete(buff)
        }
	...
	case "eos_sessions_session_search_find":

		var count = eos_sessions_session_search_get_search_result_count(global.session_search_id)
