...
{	
	case "eos_leaderboards_query_leaderboard_user_scores":
		...
		for(var a = 0 ; a < count ; a++)
		{
			...
			var ins = instance_create_depth(400,300+a*80,0,obj_eos_leaderboard_rank_friend,struct)
			...
			var scores = []
			var index = 0
			with(obj_eos_leaderboard_rank_friend)
			{
	...
	case "eos_friends_query_friends":
		...
		if(async_load[? "status"] == EpicResult.Success)
		{
			...
			eos_connect_query_external_account_mappings(global.product_user_id,0,friends_account_ids)
		}
	break
	...
	case "eos_connect_query_external_account_mappings":
		...
		{
			var user_id = eos_connect_get_external_account_mapping(global.product_user_id,friends_account_ids[a],0)
		...
		show_debug_message([global.product_user_id,"",friends_user_ids,[{stat_name: stat_selected, aggregation: agregation_seleted}],0,0])
		eos_leaderboards_query_leaderboard_user_scores(global.product_user_id,"",friends_user_ids,[{stat_name: stat_selected, aggregation: agregation_seleted}],0,0)
