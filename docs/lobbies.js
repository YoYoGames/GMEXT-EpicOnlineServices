 ...
 * @param {bool} [enable_rtc_room=false] Creates a real-time communication (RTC) room for all members of this lobby. All members of the lobby will automatically join the RTC room when they connect to the lobby and they will automatically leave the RTC room when they leave or are removed from the lobby. While the joining and leaving of the RTC room is automatic, applications will still need to use the EOS RTC interfaces to handle all other functionality for the room. The default is `false`.
 * @param {constant.EOS_RTC_JOINROOMFLAGS} [flags=EOS_RTC_JOINROOMFLAGS_ENABLE_DATACHANNEL] Flags for the local user in this room. Set to 0 to not specify any flags. See ${constant.EOS_RTC_JOINROOMFLAGS}. The default is `EOS_RTC_JOINROOMFLAGS_ENABLE_DATACHANNEL`.
 * @param {bool} [use_manual_audio_input=false] Set to `true` to enable Manual Audio Input. If manual audio input is enabled, audio recording is not started and the audio buffers must be passed manually using ${function.eos_rtc_audio_send_audio}. The default is `false`.
 * @param {bool} [use_manual_audio_output=false] Set to `true` to enable Manual Audio Output. If manual audio output is enabled, audio rendering is not started and the audio buffers must be received with ${function.eos_rtc_audio_add_notify_audio_before_render} and rendered manually. The default is `false`.
 * @param {bool} [local_audio_device_input_starts_muted=false] Set to `true` to start the audio input device's stream as muted when first connecting to the RTC room. It must be manually unmuted with a call to ${function.eos_rtc_audio_update_sending}. If manual audio output is enabled, this value is ignored. The default is `false`.
 * 
 * @returns {real}
 * 
 * @event social
