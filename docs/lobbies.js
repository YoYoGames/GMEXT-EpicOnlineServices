 ...
 * @param {constant.EOS_RTC_JOINROOMFLAGS} [flags] Flags for the local user in this room. Set to 0 to not specify any flags. See ${constant.EOS_RTC_JOINROOMFLAGS}. The default is `EOS_RTC_JOINROOMFLAGS_ENABLE_DATACHANNEL`.
 * @param {bool} [use_manual_audio_input] Set to `true` to enable Manual Audio Input. If manual audio input is enabled, audio recording is not started and the audio buffers must be passed manually using ${function.eos_rtc_audio_send_audio}. The default is `false`.
 * @param {bool} [use_manual_audio_output] Set to `true` to enable Manual Audio Output. If manual audio output is enabled, audio rendering is not started and the audio buffers must be received with ${function.eos_rtc_audio_add_notify_audio_before_render} and rendered manually. The default is `false`.
